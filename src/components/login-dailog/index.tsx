...
import { Button, Form, Image, Input, Modal, Space, Spin, message } from 'antd'
import { login, system, user } from '../../api/index'
import { getAppUrl, getHost, getMsv, setToken } from '../../utils/index'
...
export const LoginDialog: React.FC<PropInterface> = ({
  ...
  const onFinish = (values: any) => {
    ...
    if (tabKey === 0) {
      login
        .passwordLogin({ phone: values.phone, password: values.password })
        .then((res: any) => {
          setLoading(false)
  ...
  const changeQQ = () => {
    ...
    if (successRedirectUrl.includes('/#/'))
