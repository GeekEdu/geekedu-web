...
import { codeRender, getCommentTime, latexRender } from '../../utils/index'
...
function BookReadPage() {
  ...
  const getData = () => {
    ...
    bookApi.articleRead(id).then((res: any) => {
      ...
      setChapters(chaps)
      setArticles(arts)
      setArticleList(artList)
      setIsBuy(res.data.canSee)
      setCanSee(res.data.canSee)
      const pid = Number.parseInt(res.data.prevId)
      setPrevId(pid)
      const nid = Number.parseInt(res.data.nextId)
      setNextId(nid)
      setLoading(false)
    })
  }
            ...
            {!loading && canSee && (
              <div className={styles.read}>
                ...
                <div
                  ...
                  id="desc"
                  dangerouslySetInnerHTML={{ __html: list.renderContent }}
                >
                </div>
              </div>
            )}
